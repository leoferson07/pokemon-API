...
const getPokemons = async (req, res) => {
    try {
        ...
        if (name) {
            const pokeFiltered = listPokemons.filter((poke) => poke.name === name.toLowerCase())
...
const getPokemonById = async (req, res) => {
    ...
    try {
        ...
        if (id) {
            const filteredById = allPokemons.filter((obj) => obj.id == id);
...
const pokemonCreate = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json("You pokemon has be created successfully"); //Tu Pokémon ha sido creado exitosamente
